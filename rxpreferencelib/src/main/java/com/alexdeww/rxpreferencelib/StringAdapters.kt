...
object StringAdapters {
    ...
    val STRING_TO_INT = LambdaPreferenceAdapter<Int>(
            ...
            { sharedPreferences, s, defVal -> sharedPreferences.getString(s, defVal.toString()).toInt() }
    )

    val STRING_TO_LONG = LambdaPreferenceAdapter<Long>(
            { editor, key, v -> editor.putString(key, v.toString()) },
            { sharedPreferences, key, defVal -> sharedPreferences.getString(key, defVal.toString()).toLong() }
    )
